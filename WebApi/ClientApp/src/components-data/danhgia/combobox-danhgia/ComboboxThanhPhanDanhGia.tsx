...
interface IComboboxThanhPhanDanhGiaProps extends IComboboxBasePropsWithValueString {}
const getTrailingVisual = (tpdg: any) => {
  return (
    <Text
      text={tpdg.name}
      sx={{
...
const ComboboxThanhPhanDanhGia = (props: IComboboxThanhPhanDanhGiaProps) => {
  const thanhPhanDanhGia = [
    { id: 1, name: "Đánh giá chuyên cần" },
    { id: 2, name: "Đánh giá giữa kì" },
    { id: 3, name: "Đánh giá kết thúc học phần" },
  ];

  const [open, setOpen] = useState(false);
  const [filter, setFilter] = useState("");
  const dataSource = useMemo(() => {
    return thanhPhanDanhGia.map((x) => ({
      id: x.name,
      text: `${props.preText ?? ""}${x.name}`,
      search: `${x.name}`,
      trailingVisual: getTrailingVisual(x),
    }));
  }, [props.preText, thanhPhanDanhGia]);
      ...
      renderAnchor={({
        ...
        >
          ...
          >
            {selectedData
              ? selectedData.text
              : props.placeHolder ?? "Chọn thành phần đánh giá"}
          </p>
        </Button>
      )}
      title={
...
export default ComboboxThanhPhanDanhGia;
