...
interface ISelecboxMucTieuProps extends ISelectboxBaseProps {
    mucTieus: any[];
}

const SelecboxMucTieu = (props: ISelecboxMucTieuProps) => {
    const { mucTieus } = props;
    const [open, setOpen] = useState(false);
    const [filter, setFilter] = useState("");

    const dataSource = useMemo(() => {
        return mucTieus.map((x: any) => ({
            id: x.ma,
            text: `${props.preText ?? ""}${x.ma}`,
            search: `${x.ma}`,
        }));
    }, [props.preText, mucTieus]);
                ...
                >
                    ...
                    >
                        {selectedData?.length
                            ...
                            : props.placeHolder ?? "Chọn mục tiêu`"}
                    </p>
                </Button>
            )}
            title={
...
export default SelecboxMucTieu;
